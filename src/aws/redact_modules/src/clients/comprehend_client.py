...
class ComprehendClient:
    ...
    def __init__(self, region_name, s3ol_access_point: str, pii_classification_thread_count: int = CONTAINS_PII_ENTITIES_THREAD_COUNT,
        ...
        if endpoint_url is None:
            self.comprehend = boto3.client('comprehend', region_name=region_name,config=session_config)
        else:
            self.comprehend = boto3.client('comprehend', region_name=region_name, config=session_config, endpoint_url=endpoint_url, verify=False)
